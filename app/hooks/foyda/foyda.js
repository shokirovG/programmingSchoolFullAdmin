...
function foyda(students, workers, chiqimlar = [],majburiyChiqimlar=[]) {
  let totalBalans = 0;
  let totalMajburiy = 0
  ...
  for (let worker of workers) {
    ...
    for (let teacherGroup of teacherGroups) {
      ...
      const balans = filterGroupsStudents.reduce((s, {price,foiz,department}) => {
        return s + Number(price)+calcPrice(price, foiz, department);
      }, 0);
  ...
  return totalBalans + eskiOyBalans - totalMajburiy;
}
